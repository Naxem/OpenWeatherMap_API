...
function requete($nbAction, $cleApi, $latitude, $longitude)
{
    ...
    if ($nbAction == '1') {
        ...
        $urlApiOpenWeatherMap = "https://api.openweathermap.org/data/3.0/weather?lat={$latitude}&lon={$longitude}&appid={$cleApi}&lang=fr";
        ...
        $forecastApiUrl = "https://api.openweathermap.org/data/3.0/onecall?lat={$latitude}&lon={$longitude}&exclude=current,minutely,hourly&appid={$cleApi}&lang=fr";
        $reponseApiOpenWeatherMap = file_get_contents($forecastApiUrl);
    } elseif ($nbAction == '3') {
        // Affichage de la météo par heure pour aujourd'hui et demain
        $dailyWeatherApiUrl = "https://api.openweathermap.org/data/3.0/forecast?lat={$latitude}&lon={$longitude}&appid={$cleApi}&lang=fr";
